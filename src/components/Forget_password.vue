 ...
 <div class="middle-box text-center">
    ...
    <div style="width:8.33rem;margin:0 auto;padding-top:3.08rem" class="reg">
        ...
        <div class="form-group">
            ...
            <span style="color:red;width:32%">{{mobilemessage}}</span>
        </div>
        <div class="form-group">
            <input type="password" class="form-control" v-model="code" placeholder="短信验证码" required="" style="padding:0 0.23rem">
            <span @click="requirecode()" v-if="type==1">获取验证码</span>
            <span  v-else>{{codemessage}}</span>
        </div>  
        <div class="form-group">
            <input type="password" class="form-control" v-model="password" placeholder="请输入密码" required="" style="padding:0 0.23rem;width:32%">
            <span style="color:red;width:28%">{{pwdmessage}}</span>
        </div> 
       ...
       toColor:function(index){
            ...
            if(that.mobile==""){
               ...
               if(reg.test(that.mobile)&&that.password!==""){
                   ...
                   that.$Post('/resets' ,{mobile: that.mobile,code:that.code, pwd:that.password,type:4}, function(data){
                       console.log(data);
                     
                       if(data.code=="221"){
//                         alert("设置失败");
							mizhu.alert('温馨提示', '设置失败');
                       }else{
            ...
            if(that.mobile!==""){
               ...
               if(reg.test(that.mobile)){
                  ...
                  that.mobilemessage="";
                    ...
                    that.codemessage=60;
                    that.$Post('/smscode' ,{mobile: that.mobile,type:"4"}, function(data){
                        console.log(data);
                          if(data.code=="203"){
